...
#loss_fn = nn.CrossEntropyLoss()
    ...
    _epoch_list = []
    for (path, dir, files) in os.walk('./states_12/'):
    ...
    if len(files) >= 1:
        torchstatefile = './states_12/' + sorted(files)[-1]
    ...
    for epoch in range(epochs):
        ...
        #train_losses = np.load('./losses/test12_train_losses.npy')
        #valid_losses = np.load('./losses/test12_valid_losses.npy')
        ...
        _rad = np.array(rad)
        _albwf = np.array(albwf)
        _o3wf = np.array(o3wf)

        def normalize_radiance(_rad):
            min_at_wavs = []
            max_at_wavs = []
            #print(rad[0][0][0][0])
            for iwav in range(len(wav)):
                min_at_wav = np.min(rad[0, 0, 0, 0, :, iwav])
                max_at_wav = np.max(rad[0, 0, 0, 0, :, iwav])
                rad[0, 0, 0, 0, :, iwav] = (rad[0, 0, 0, 0, :, iwav]
                        - min_at_wav)/(max_at_wav - min_at_wav)
                min_at_wavs.append(min_at_wav)
                max_at_wavs.append(max_at_wav)
                #print(min_at_wav, max_at_wav)
            min_at_wavs = np.array(min_at_wavs)
            max_at_wavs = np.array(max_at_wavs)
            return rad, min_at_wavs, max_at_wavs
        _rad_norm, _min_at_wavs, max_at_wavs = normalize_radiance(_rad)
        #print(_min_at_wavs, _max_at_wavs)
        ...
        train_loader = XY_data_loader_toz_800_v2_sza_train(pre_list_train,
                alb_list_train, raa_list_train, vza_list_train, sza_list_train,
                toz_list_train, 
                _rad_norm[0, 0, 0, 0, sza_train_idx, :], 
                albwf[0][0][0][0][sza_train_idx][:], 
                o3wf[0][0][0][0][sza_train_idx][:])
        ...
        test_loader = XY_data_loader_toz_800_v2_sza_test(pre_list_test,
                alb_list_test, raa_list_test, vza_list_test, sza_list_test, 
                toz_list_test, 
                _rad_norm[0, 0, 0, 0, sza_test_idx, :], 
                albwf[0][0][0][0][sza_test_idx][:], 
                o3wf[0][0][0][0][sza_test_idx][:])

        print(_rad_norm[0,0,0,0, 92, :])
        print(rad[0,0,0,0,92,:])
        ...
        for i, (features, radiances) in enumerate(train_loader):
            print('train', i, features.shape, radiances)
            #if real_epoch == 0:
            optimizer.zero_grad()
            ...
            #for i in range(12):
                #outputs[i, :] = (outputs[i, :] * 
                    #(np.array(_max_at_wavs[660:]) - 
                        #np.array(_min_at_wavs[660:]))  +
                    #np.array(_min_at_wavs[660:]))
        ...
        with torch.no_grad():
            for i, (features, radiances) in enumerate(test_loader):
                ...
                #print('test', outputs, radiances)
                #print(outputs.shape, radiances.shape)
                #print(_rad[0, 0, 0, 0, sza_test_idx, :])
                #print(radiances[-1])
                #print(len(radiances))
                #quit()
                #valid_losses.append(loss.data)
        ...
        #np.save('./train_losses/test12_train_losses.npy',
                #np.array(train_losses), allow_pickle=True)
        #np.save('./valid_losses/test12_valid_losses.npy',
...
#test_loader
        ...
        lossesfile = './result/12_log_mse_rtm_nn_sza_test_mean_losses.txt' 
        #print('lossesfile')
        ...
        torchstatefile = './states_12/12_log_mse_rtm_nn_sza_test_epoch_' + str(epoch_local).zfill(8) + '.pth'
        if epoch_local % 10 == 0:
