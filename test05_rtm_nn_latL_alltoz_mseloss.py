...
from ds_rtm_nn import XY_data_loader_toz_800_train_valid
...
#loss_fn = nn.CrossEntropyLoss()
    ...
    _epoch_list = []
    for (path, dir, files) in os.walk('./states_05/'):
    ...
    if len(files) >= 1:
        torchstatefile = './states_05/' + sorted(files)[-1]
...
#for epoch in range(epochs):
    ...
    #train_losses = np.load('./losses/test05_train_losses.npy')
    #valid_losses = np.load('./losses/test05_valid_losses.npy')
    ...
    for LUT_file in LUT_filelist:
            ...
            train_loader, valid_loader = XY_data_loader_toz_800_train_valid(pre_list, alb_list,
            ...
            #np.save('./train_losses/test05_train_losses.npy',
                    #np.array(train_losses), allow_pickle=True)
            #np.save('./valid_losses/test05_valid_losses.npy',
...
#test_loader
    #for batch_idx, (f_plot, r_plot) in enumerate(test_loader):
        ...
        ##if i % 10 == 0:
            ##print(i, outputs)
        #filename = ('./plot/05_rtm_nn_' + lat + '_alltoz_epoch_' + str(epoch).zfill(5) +
    ...
    lossesfile = './result/05_rtm_nn_latL_tozall_mseloss_mean_losses.txt' 
    print('lossesfile')
    ...
    torchstatefile = './states_05/05_rtm_nn_latL_tozall_mseloss_epoch_' + str(epoch).zfill(5) + '.pth'
    print('torchstatefile')
