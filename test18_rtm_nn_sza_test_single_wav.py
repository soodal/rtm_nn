...
class MLPv03_single_wave(nn.Module):
    def __init__(self):

        linear1 = nn.Linear(7, 200)
        ...
        super(MLPv03_single_wave, self).__init__()
...
from ds_rtm_nn import features_maker_toz_single_wav
#from ds_rtm_nn import XY_data_loader_toz_v3_sza_radlog
from ds_rtm_nn import XY_data_loader_single_wav_train
...
#loss_fn = nn.CrossEntropyLoss()
    ...
    for epoch in range(epochs):
        ...
        (pre_list_train, alb_list_train, raa_list_train, vza_list_train,
                sza_list_train, toz_list_train, wav_list_train) = features_maker_toz_single_wav(
                pre, alb, raa, vza, sza[sza_train_idx], toz, wav)    

        (pre_list_test, alb_list_test, raa_list_test, vza_list_test,
                sza_list_test, toz_list_test, wav_list_test) = features_maker_toz_single_wav(
                pre, alb, raa, vza, sza[sza_test_idx], toz, wav)

        #(pre_list, alb_list, raa_list, vza_list, sza_list, 
                #toz_list, wav_list) = features_maker_toz_single_wav(pre, alb, raa, 
                        #vza, sza[sza_train_idx], toz, wav)    

        #(pre_list, alb_list, raa_list, vza_list, sza_list, 
                #toz_list, wav_list) = features_maker_toz_single_wav(pre, alb, raa, 
                        #vza, sza[sza_test_idx], toz, wav)    

        #(pre_list_test, alb_list_test, raa_list_test, vza_list_test,
                #sza_list_test, toz_list_test) = features_maker_toz(
                #pre, alb, raa, vza, sza[sza_test_idx], toz)    
    
        #train_loader = XY_data_loader_single_wav_train(pre_list, alb_list,
                #raa_list, vza_list, sza_list, toz_list, wav_list, rad, albwf, o3wf)

        train_loader = XY_data_loader_single_wav_train(pre_list_train,
                alb_list_train, raa_list_train, vza_list_train, sza_list_train,
                toz_list_train, wav_list_train, 
                rad[0][0][0][0][sza_train_idx][:], 
                albwf[0][0][0][0][sza_train_idx][:], 
                o3wf[0][0][0][0][sza_train_idx][:])

        test_loader = XY_data_loader_single_wav_train(pre_list_test,
                alb_list_test, raa_list_test, vza_list_test, sza_list_test, 
                toz_list_test, wav_list_test, 
